...
func _Gov_required_if(field1 string, _ any, field2 string, value2 any, cond any) error {
	v2, c := cast.ToString(value2), cast.ToString(cond)
	if v2 != c {
		return _Gov_Error("required_if", field1, "", field2, v2)
	}
	return nil
}
...
func _Gov_size_int64(field string, value int64, cond int64) error {
	if value != 10 {
		return _Gov_Error("size", field, cast.ToString(value), "", "")
	}
	return nil
}
...
func NewProfileSchema(u Profile) ProfileSchema {
...
func (s ProfileSchema) Validate() (messages []string) {
	for _, rule := range s.rules {
		if err := rule.Validate(); err != nil {
			messages = append(messages, err.Error())
		}
